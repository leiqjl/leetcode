...
public class DominoAndTrominoTiling {
    ...
    //1. F(N-1)=F(N-2)+F(N-3)+F_up(N-2)+F_down(N-2)
    //2. F(N)-F(N-1)=F(N-1)+F(N-2)+F_up(N-1)+F_down(N-1) - (F(N-2)+F(N-3)+F_up(N-2)+F_down(N-2))
    //              = F(N-1)+F(N-2) + F_down(N-2)+F(N-3) + F_up(N-2)+F(N-3) - (F(N-2)+F(N-3)+F_up(N-2)+F_down(N-2))
    //              = F(N-1)+ F(N-3)
    //3. F(N)= 2F(N-1)+F(N-3)
